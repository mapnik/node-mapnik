...
if __name__ == '__main__':
    ...
    if ip:
        ...
        settings_dict['input_plugins'] = '\'%s\'' % os.popen("./scripts/mapnik-config.sh --input-plugins").readline().strip()
    
    mf = os.environ.get('MAPNIK_FONTS')
    if mf:
        settings_dict['fonts'] = mf
    else:
        settings_dict['fonts'] = '\'%s\'' % os.popen("./scripts/mapnik-config.sh --fonts").readline().strip()
