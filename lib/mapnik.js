...
exports.shapeindex = function(args, stdout, stderr, callback) {
    callback = callback || function() {};
    var shapeindex = path.join(path.dirname(binding_path), 'shapeindex');

    var proc = spawn(shapeindex, args)
        .once('error', function(err) { callback(err, undefined); })
        .on('close', function(code) { callback(null, code); });
