...
struct p2p_result {
    explicit p2p_result() : distance(-1),
                            x_hit(0),
                            y_hit(0) {}
...
VectorTile::VectorTile(std::uint64_t z,
                       ...
                       std::int32_t buffer_size) : Nan::ObjectWrap(),
...
NAN_METHOD(VectorTile::layer) {
    ...
    if (maybe_local.IsEmpty())
        Nan::ThrowError("Could not create new Layer instance");
    else
...
struct vector_tile_render_baton_t {
    ...
    bool zxy_override;
    bool error;
    vector_tile_render_baton_t() : request(),
                                   m(nullptr),
                                   d(nullptr),
                                   surface(),
                                   variables(),
...
struct baton_guard {
    baton_guard(vector_tile_render_baton_t* baton) : baton_(baton),
                                                     released_(false) {}
