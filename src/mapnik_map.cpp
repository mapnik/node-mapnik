...
void Map::Initialize(Handle<Object> target) {
    ...
    constructor->PrototypeTemplate()->Set(String::NewSymbol("layers"), meta);
    */
        
    //constructor->PrototypeTemplate()->SetNamedPropertyHandler(property);
...
/*
Handle<Value> Map::property(Local<String> attr,
...
PixelArray *arr = imageData->pixelArray();
  ...
  Layer* m = ObjectWrap::Unwrap<Layer>(args[0]);
  //map.addLayer(lyr);
